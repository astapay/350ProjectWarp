...
public class PlayerController : FighterController
{
    ...
    private Vector3 lastTouchedGround;
    ...
    void Update()
    {
        // Play the death animation if we are dead
        if (hp == 0)
        {
            DeathAnimation();
        }
        // Check if we have paused
        else if (!gm.TimeFrozen())
        {
            ...
            if (exitStarted)
            {
                ...
                // Play the exit sprites
                // (This is similar to UpdateAnimation)
                gameObject.GetComponent<Animator>().enabled = false;

                currentSprite++;

                // If we are out of the scene, we display our results and move on
            ...
            // Check to see if we have entered the scene
            else if (doneWithEntrance)
            {
                // This will be used to put us back on the stage if we fall
                if (isGrounded)
                {
                    lastTouchedGround = transform.position;
                }
                ...
                // Check to see if we are not getting hit
                if (hitstunFrames <= 0)
                {
                ...
                {
                    // Reduce our hitstun frames otherwise
                    hitstunFrames--;
                }
            ...
            // Otherwise, we are currently entering the scene
            else
            {
                currentSprite++;

                // Play our entranceSprites
                // If we are done, mark it so
    ...
    private void UpdateAnimation()
    {
        ...
        // Check to see if we are getting hit
        if (hitstunFrames <= 0)
        {
        ...
        // If we are getting hit, play the correct sprite
        else
        {
            s = deathSprites[7];
            hitstunFrames--;
    ...
    private void DeathAnimation()
    {
        ...
        {
            // After a short delay, display the game over screen
            if (currentSprite == 40)
            {
                gm.GameOver();
            }
    ...
    private void SpecialStart()
    {
        ...
        if (isAttacking)
        {
            ...
            if (attackType == 13 && activeFrameCounter > 0)
            {
                ...
                // Checks for teleporting in a certain direction
                if (isMoving)
                {
                    // The teleportation is based on the unit circle
                    float moveDir = move.ReadValue<float>();
                ...
                // Double the range so that we're not stingy
                xOffset *= 2;
                yOffset *= 2;

                // Teleport that direction
                transform.position = new Vector3(currentX + xOffset, currentY + yOffset, 0);

                // We have to do the same to our saveTransform
            ...
            {
                // Logic for creating projectiles
    ...
    // <summary>
    // Increases the number of portal gems we've collected
    // </summary>
    ...
    private void Handle_MoveStart(InputAction.CallbackContext obj)
    {
        ...
        if (hitstunFrames <= 0)
        {
            ...
            // as long as we are not crouching or are not grounded
    ...
    {
        // This is used for the teleportation special
        isJumping = true;
    ...
    {
        // We want to crouch
        // As long as we are not getting hit
        if (hitstunFrames <= 0)
        {
            isCrouching = true;
        }
    ...
    private void Handle_Normal(InputAction.CallbackContext obj)
    {
        ...
        if (hitstunFrames <= 0)
        {
            ...
            if (nChain < 3)
            {
                if (canAttack)
                {
                    // Undo adjusting the transforms for problematic animations
                    if (attackType >= 1 && attackType <= 3 || attackType == 7 || attackType == 15)
                    {
                        UndoAdjustTransform();
                    }
    ...
    {
        // If we have enough Portal Gems,
        // we can exit the level
        if ((SceneManager.GetActiveScene().name == "Level1" && gemCount >= 3)
                || (SceneManager.GetActiveScene().name == "Level2" && gemCount >= 9))
        {
            // Set variables
            exitStarted = true;
            currentSprite = -1;
        ...
        {
            // If we can attack in the first place...
            if (canAttack)
            {
                // Start with standing
                attackType = 13;
